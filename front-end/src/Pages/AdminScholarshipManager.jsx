...
const AdminScholarshipManager = () => {
  ...
  const [currentStudentScholarshipsPage, setCurrentStudentScholarshipsPage] =
    useState(1);
  ...
  const filteredScholarships = useMemo(() => {
    return scholarships.filter((scholarship) => {
      ...
      if (scholarshipSearchTerm) {
        ...
        if (
          !(
            scholarship.scholarshipId.toLowerCase().includes(term) ||
      ...
      if (
        ...
        !scholarship.scholarshipType.includes(
          scholarshipFilters.scholarshipType
        )
      ) {
        return false;
      ...
      if (
        scholarshipFilters.minAmount &&
        scholarship.amount < parseFloat(scholarshipFilters.minAmount)
      ) {
        return false;
      }

      if (
        scholarshipFilters.maxAmount &&
        scholarship.amount > parseFloat(scholarshipFilters.maxAmount)
      ) {
        return false;
      }
  ...
  const filteredStudentScholarships = useMemo(() => {
    return studentScholarships.filter((assignment) => {
      ...
      if (studentSearchTerm) {
        ...
        const scholarshipType = getScholarshipType(
      ...
      if (studentFilters.student) {
        ...
        const studentIdMatch = assignment.studentId
          .toLowerCase()
          .includes(studentTerm);
        const studentNameMatch = getStudentName(assignment.studentId)
          .toLowerCase()
          .includes(studentTerm);
  ...
  const getCurrentScholarships = () => {
    ...
    return filteredScholarships.slice(
  ...
  const getCurrentStudentScholarships = () => {
    ...
    return filteredStudentScholarships.slice(
      indexOfFirstRecord,
      indexOfLastRecord
    );
  };
  ...
  const paginateStudentScholarships = (pageNumber) =>
    setCurrentStudentScholarshipsPage(pageNumber);
  ...
  const nextStudentScholarshipsPage = () => {
    if (
      currentStudentScholarshipsPage <
      Math.ceil(filteredStudentScholarships.length / itemsPerPage)
    ) {
      setCurrentStudentScholarshipsPage(currentStudentScholarshipsPage + 1);
    }
  };
  ...
  const assignScholarshipToStudent = async () => {
    ...
    if (
      assignData.scholarshipPercentage < 0 ||
      assignData.scholarshipPercentage > 100
    ) {
    ...
    try {
      if (confirmAction === "deleteScholarship") {
        ...
        const totalPages = Math.ceil(
          (filteredScholarships.length - 1) / itemsPerPage
        );
        ...
        await axiosInstance.delete(
          `/admin/student-scholarships/${itemIdToDelete}`
        );
        alert("Scholarship assignment removed successfully!");
        // If we're on the last page and it becomes empty, go to previous page
        const totalPages = Math.ceil(
          (filteredStudentScholarships.length - 1) / itemsPerPage
        );
  ...
  const scholarshipsTotalPages = Math.ceil(
    filteredScholarships.length / itemsPerPage
  );
  const studentScholarshipsTotalPages = Math.ceil(
    filteredStudentScholarships.length / itemsPerPage
  );
      ...
      <div className="bg-white rounded-lg shadow-lg p-4">
        ...
        <div className="flex justify-between items-center mb-6 gap-3">
          <div>
            ...
            <p className="text-gray-600 xs:text-xs md:text-sm">
          ...
          <div className="flex gap-2">
            ...
            >
              <Plus size={20} />
              <h1 className="xs:hidden md:block">{showCreateForm ? "Cancel" : "Create New"}</h1>
            </FormButton>
          </div>
        </div>
        ...
        <div className="space-y-4 mb-8">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {scholarshipFilters.scholarshipType && (
                  <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                    ...
                    <button
                      onClick={() =>
                        handleScholarshipFilterChange("scholarshipType", "")
                      }
                      className="ml-1 inline-flex h-4 w-4 rounded-full items-center justify-center text-blue-400 hover:bg-blue-200 hover:text-blue-500 focus:outline-none"
                    >
                      <X className="h-3 w-3" />
                ...
                {(scholarshipFilters.minAmount ||
                  scholarshipFilters.maxAmount) && (
                  <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                    Amount: {scholarshipFilters.minAmount || "0"} -{" "}
                    {scholarshipFilters.maxAmount || "∞"}
                    <button
                      onClick={() => {
          ...
          {filteredScholarships.length === 0 ? (
            <div className="text-center py-12 bg-gray-50 rounded-lg">
              ...
              <p className="text-gray-400 text-sm mt-2">
                Click "Create New Scholarship" to add your first scholarship
              </p>
            </div>
          ) : (
            <>
              ...
              <div className="block md:hidden space-y-4">
                {currentScholarships.map((scholarship) => (
                  <div
                    key={scholarship.scholarshipId}
                    className="bg-white rounded-lg shadow border border-gray-100 p-4 hover:shadow-md transition"
                  >
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <h2 className="text-xl font-semibold text-gray-700 border-b-2 border-gray-200 pb-2">
              Student Scholarship Assignments (
              {filteredStudentScholarships.length})
            </h2>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
              ...
              <div>
                <input
                  ...
                  onChange={(e) =>
                    handleStudentFilterChange("student", e.target.value)
                  }
          ...
          {filteredStudentScholarships.length === 0 ? (
            <div className="text-center py-8 bg-gray-50 rounded-lg">
              ...
              <p className="text-gray-400 text-sm mt-2">
                Click "Assign to Student" to assign scholarships to students
              </p>
            </div>
          ) : (
            <>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {currentStudentScholarshipRecords.map((assignment) => (
                      <tr key={assignment.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          {getScholarshipType(assignment.scholarshipId)} (
                          {assignment.scholarshipId})
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          <button
                            onClick={() =>
                              deleteStudentScholarship(assignment.id)
                            }
                            disabled={deleting} // 禁用按钮当正在删除时
                            className={`p-2 rounded-full ${
                              deleting
              ...
              {studentScholarshipsTotalPages > 1 && (
                <div className="flex justify-center mt-6">
                  <nav className="flex items-center gap-2">
                    ...
                    {[...Array(studentScholarshipsTotalPages)].map(
                      (_, index) => {
                        const pageNumber = index + 1;
                        return (
                          <button
                            key={pageNumber}
                            onClick={() =>
                              paginateStudentScholarships(pageNumber)
                            }
                            className={`w-10 h-10 rounded-full ${
        ...
        <CustomConfirmDialog
          ...
          title={
            confirmAction === "deleteScholarship"
              ? "Delete Scholarship"
