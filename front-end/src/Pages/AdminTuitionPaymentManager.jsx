...
const AdminTuitionPaymentManager = () => {
  ...
  const filteredTuitionPayments = useMemo(() => {
    return tuitionPayments.filter((payment) => {
        ...
        if (
          !(
            ...
            (payment.scholarshipId &&
              payment.scholarshipId.toLowerCase().includes(term)) ||
      ...
      if (filters.student) {
        ...
        const studentIdMatch = payment.studentId
          .toLowerCase()
          .includes(studentTerm);
        const studentNameMatch = getStudentName(payment.studentId)
          .toLowerCase()
          .includes(studentTerm);
      ...
      if (
        ...
        payment.paymentStatus.toString() !== filters.paymentStatus
      ) {
        return false;
      }
      ...
      if (
        ...
        payment.paymentMethod.toString() !== filters.paymentMethod
      ) {
        return false;
      }
      ...
      if (
        filters.minAmount &&
        payment.amountPaid < parseFloat(filters.minAmount)
      ) {
        return false;
      }
      ...
      if (
        filters.maxAmount &&
        payment.amountPaid > parseFloat(filters.maxAmount)
      ) {
        return false;
      }
  ...
  const getCurrentTuitionPayments = () => {
    ...
    return filteredTuitionPayments.slice(
      indexOfFirstPayment,
      indexOfLastPayment
    );
  };
  ...
  const nextPage = () => {
    if (
      currentPage < Math.ceil(filteredTuitionPayments.length / itemsPerPage)
    ) {
      setCurrentPage(currentPage + 1);
    }
  };
  ...
  const saveEdit = async () => {
    try {
      const response = await axiosInstance.put(
        `/admin/tuition-payments/${editingId}`,
        {
          ...editData,
        }
      );
  ...
  const confirmDelete = async () => {
    ...
    try {
      await axiosInstance.delete(
        `/admin/tuition-payments/${paymentIdToDelete}`
      );

      alert("Tuition payment record deleted successfully!");
      // If we're on the last page and it becomes empty, go to previous page
      const totalPages = Math.ceil(
        (filteredTuitionPayments.length - 1) / itemsPerPage
      );
        ...
        <div className="flex justify-between items-center mb-6 gap-3">
          <div>
            ...
            <p className="text-gray-600 xs:text-xs md:text-sm">
              Manage student tuition payments • Total:{" "}
              {filteredTuitionPayments.length}
            </p>
          </div>
              ...
              <div className="p-6">
                <div className="flex justify-between items-center mb-4">
                  <h2 className="text-xl font-bold text-gray-800">
                    Create New Tuition Payment Record
                  </h2>
                  <button
                  ...
                  <FormGroup>
                    ...
                    <FormInput
                      ...
                      onChange={(e) =>
                        setCreateData({
                          ...createData,
                          amountPaid: parseFloat(e.target.value) || 0.0,
                        })
                      }
        ...
        <div className="space-y-4">
          ...
          {filteredTuitionPayments.length === 0 ? (
            <div className="text-center py-12 bg-gray-50 rounded-lg">
              ...
              <p className="text-gray-400 text-sm mt-2">
                Click "Create New Record" to add your first record
              </p>
            </div>
          ) : (
            <>
              ...
              <div className="rounded-lg shadow overflow-hidden xs:block md:hidden">
                {/* ✅ Desktop Table View */}
                  ...
                  <ModernForm>
                    ...
                    <FormRow>
                      <FormGroup>
                        ...
                        <FormSelect
                          value={
                            editData.paymentStatus !== undefined
                      ...
                      <FormGroup>
                        ...
                        <FormSelect
                          value={
                            editData.paymentMethod !== undefined
